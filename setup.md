...
Every time something is saved, a small build script (tools/build\_site.py) writes out every page as plain HTML, including one page per artwork (art/tryst.html and so on), plus the sitemap and search data. Then GitHub publishes it. You never run this yourself; preview.bat runs it for you on your computer.



Don't edit the .html files by hand. They're regenerated on every publish. Change the content in the editor, or the layout in tools/build\_site.py, assets/css/style.css and assets/js/site.js.
...
Make a GitHub account for the site, or use yours. Turn on two-factor sign-in straight away (Settings → Password and authentication; a passkey is best).

Create a new repository called mjartist. Private is fine.

Upload everything in this folder, including the hidden .github folder and .pages.yml. The easy way is GitHub Desktop: add the folder as a repository, then publish it.

3\. Turn on hosting (GitHub Pages)

In the repo, go to Settings → Pages → Build and deployment → Source: GitHub Actions.

Open the Actions tab. "Publish site" runs on every change. When it's green, the site is live at https://<your-username>.github.io/mjartist/.

Custom domain: go to Settings → Pages → Custom domain → www.mjartist.com. Then, wherever the domain's DNS is managed:
...
Verify the domain (this blocks anyone else from hijacking it on GitHub): your profile Settings → Pages → Add a domain → add the TXT record GitHub gives you.
...
Michael makes his own free GitHub account, with two-factor sign-in on.

Repo → Settings → Collaborators → invite him (Write access). This gives him this one repository only.

Go to app.pagescms.org and sign in with GitHub. When it asks, install the Pages CMS app on only the mjartist repository, not "All repositories".

Michael bookmarks app.pagescms.org, signs in and picks mjartist. He'll see three sections:
...
Press Save. The site updates in about a minute.
...
Google Search Console (search.google.com/search-console): add mjartist.com as a Domain property, verify it with the TXT record it gives you (added in Wix → Domains → DNS), then go to Sitemaps and submit https://www.mjartist.com/sitemap.xml.

Bing Webmaster Tools: sign in and choose "Import from Google Search Console". It takes two minutes.

Ask for links: Art Junction, Surrey Sculpture Society, Hannah Peschar, Oxmarket Gallery, the Guild of Aviation Artists and each exhibition venue.

Keep filling in details: medium, size, year and a sentence of notes on the key works. Search engines read those words.
...
Get a free access key at web3forms.com, using Michael's email address.

Paste it into the editor: About, contact \& home page → Contact form key.
...
Two hidden "honeypot" fields that people never see but bots fill in. Those messages are silently discarded, and the bot is shown a normal "thank you" so it doesn't try again.

A time check. A form completed in under 4 seconds is treated as a bot.

Content checks. Messages with HTML or link code, more than two web links, a missing message or an invalid email are stopped, with a plain explanation for real visitors.

One message a minute from the same browser.

Web3Forms' own server-side spam filter screens everything that reaches them.

An optional "I am human" check (hCaptcha). Tick Contact form — add "I am human" check in the editor if spam still gets through. It adds one small click for visitors, so it's off by default.
...
Sign up free at behold.so and connect Michael's Instagram account.

Create a feed, choose JSON as the type, and copy its address (it looks like https://feeds.behold.so/AbC123).

Paste it into the editor: About, contact \& home page → Instagram feed.
